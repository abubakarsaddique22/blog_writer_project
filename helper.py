...
medium_article_team = Team(
    ...
    num_history_runs=10,
    tools=[LocalFileSystemTools(target_directory=medium_dir,
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Then change this part in your CMD loop
        safe_prompt = sanitize_filename(user_prompt)
        agent_filename = f"{agent.id}_{safe_prompt[:50]}.txt"  # use sanitized prompt
        filepath = research_dir / agent_filename
...
write a clear, engaging, well-structured Medium-style article about: {user_prompt}. 
Do NOT include raw tables, links, or PDF text. Only narrative suitable for a blog.
Include:
1. Introduction
2. Key applications / examples
3. Benefits
4. Challenges / Risks
5. Future outlook
6. Conclusion

Make it readable for general audience, professional and informative.
